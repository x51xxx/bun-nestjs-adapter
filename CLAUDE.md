...
Runtime: **Bun ≥ 1.2.0** (CI pins 1.3.5). This is a Bun project — use `bun`, not `npm`/`node`.
...
**Microservices** (`src/microservices/`): two transports. **TCP** — `BunServerTcp` (a `CustomTransportStrategy` on `Bun.listen()`) and `BunClientTcp` (a `ClientProxy` on `Bun.connect()`); both wrap `BunJsonSocket`, which reimplements Nest's `JsonSocket` length-prefixed framing (`<charLength>#<json>`, where the prefix is the JSON string's **character** length, not byte length — receiving uses a streaming `TextDecoder` so a multibyte sequence split across TCP reads is reassembled before the length compare). **Redis** — `BunServerRedis`/`BunClientRedis` on `Bun.RedisClient` Pub/Sub, using Nest's channel scheme (pattern = request channel, `<pattern>.reply` = response channel); two connections each because a subscribed Redis client may only (un)subscribe/ping. Staying byte-exact to these formats is what makes the transports interoperate with stock Nest `ServerTCP`/`ClientTCP` and `ServerRedis`/`ClientRedis`. This code imports `@nestjs/microservices` (another *optional* peer), so it ships from a **separate entry point** (`src/microservices/index.ts` → `@trishchuk/bun-nestjs-adapter/microservices`, wired in `tsup.config.ts` + `package.json` `exports`). Never re-export it from `src/index.ts` — that would pull `@nestjs/microservices` into the main HTTP/WS bundle.
