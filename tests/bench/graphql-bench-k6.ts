/**
 * GraphQL benchmark runner — k6 edition.
 *
 * Drop-in replacement for `graphql-bench.ts`: identical CLI flags and identical
 * `--json-out` shape, so `run-matrix.ts` consumes it unchanged. Load is
 * generated by an EXTERNAL `k6` process (Go, multi-threaded) instead of an
 * in-process Bun `fetch` client, so REST and GraphQL use the *same* instrument
 * (no methodological asymmetry).
 *
 * For GraphQL the load generator choice barely moves the numbers (per-request
 * server work dominates, so client contention is a small fraction — measured
 * within noise vs the in-process client), but using k6 everywhere keeps one
 * code path and one honest story. The queries match `graphql-bench.ts` exactly.
 *
 * Server-side RSS is still sampled here via `ps -o rss=` against the spawned
 * target child; k6 only drives traffic.
 *
 * Requires k6 on PATH (`brew install k6`). The actual mix + query payloads
 * live in `graphql-k6.js`.
 */
...
// Default matrix (cross-adapter Apollo comparison).
...
// Selectable via `--only` but excluded from the default run:
...
const K6_SCRIPT = join(import.meta.dir, 'graphql-k6.js');

const OP_KINDS = ['list', 'byId', 'create', 'merge'] as const;
type OpKind = (typeof OP_KINDS)[number];

// Mirrors graphql-bench.ts so the report's mix table is identical.
...
function printHelp() {
  console.log(`GraphQL benchmark runner (k6)

Usage: bun run tests/bench/graphql-bench-k6.ts [options]
...
Options:
  ...
  -d, --duration <sec>    Measurement window in seconds (default ${DEFAULTS.duration})
      ...
      --only <list>       Comma-separated subset, e.g. --only graphql-nest-bun,graphql-nest-bun-yoga
...
async function waitReady(
  ...
  const start = Date.now();
  let lastErr: unknown;
  const probeUrl = `${baseUrl}/graphql`;
  while (Date.now() - start < timeoutMs) {
    ...
    try {
      // GraphQL endpoints answer a POST; `{__typename}` is always valid.
...
async function main() {
  ...
  const k6Version = (spawnSync('k6', ['version'], { encoding: 'utf8' }).stdout || '')
    .trim()
    .split('\n')[0];
  console.log(`GraphQL benchmark runner (load: ${k6Version || 'k6'})`);

  const tmpDir = mkdtempSync(join(tmpdir(), 'graphql-k6-'));
  ...
  if (targets.includes('graphql-nest-bun') && targets.includes('graphql-nest-bun-yoga')) {
    compare(
      'GRAPHQL NEST-BUN (apollo) vs NEST-BUN-YOGA',
      results['graphql-nest-bun'],
      results['graphql-nest-bun-yoga'],
    );
  }
